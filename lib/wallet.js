...
WalletController.prototype.sendTx = function (req, res) {
	...
	expectedFee = BigNumber(expectedFee);
	var utxoCapBN = amountBN.plus(expectedFee);
...
WalletController.prototype.createUnsigedRawTransaction = function (req, res) {
	...
	var fromAddrString = req.body.from_public_address,
		...
		subtractFeeFromAmount = req.body.subtract_fee_from_amount;
	...
	if (!customFee || customFee < 0) {
		//customFee = 0; 
		customFee = defaultWebWalletFee;	//WL should remove when update FWW
	}
	...
	if (subtractFeeFromAmount && subtractFeeFromAmount > 0) {
		subtractFeeFromAmount = 1; // set subtract fee from amount
	} else {
		subtractFeeFromAmount = 0; // set subtract fee from amount
	}
	...
	//Check Dust inputs
	if (amountBN.isLessThan(bitcore.Transaction.DUST_AMOUNT)) {
		return self.common.handleErrors({
			message: 'Sending amount is less than DUST_AMOUNT: ' + bitcore.Transaction.DUST_AMOUNT,
			code: 1
		}, res);
	}
	...
	this.node.estimateFee(nConfirmExpect, function (err, feeRateEstimate) {
		if (err) {
			console.log('estimateFee Error', err)
			//return self.common.handleErrors(err, res);
		} else {
		...
		if (customFeeBN.isGreaterThan(0)) {
			...
			expectedFee = customFeeBN.toNumber();
		} else {
			transaction = transaction.feePerKb(_feePerKb);
			expectedFee = transaction._estimateFee() / 2;
		}
		...
		if(subtractFeeFromAmount){
			...
			utxoCapBN = amountBN.plus(expectedFee);
		}
		...
		self.node.getAddressUnspentOutputs(fromAddrString, true, function (err, utxos) {
			...
			utxos.forEach(function (tx) {
				...
				if (tx.confirmations > 0) {
					if (totalBN.isLessThan(utxoCapBN)) {
						...
						if (remainBN.isGreaterThan(0) && remainBN.isLessThan(bitcore.Transaction.DUST_AMOUNT)) {
							suggest_utxos.push(tx);
							totalBN = totalBN.plus(tx.satoshis);
						}
					}
			...
			if(totalBN.isLessThan(amountBN)){
				return self.common.handleErrors({ message: "Not enough money", code: 1 }, res);
			}

			if (totalBN.isLessThan(utxoCapBN) && customFeeBN.isGreaterThan(0)) {
				return self.common.handleErrors({ message: "Not enough money for transaction fee", fee: customFeeBN.toNumber(), code: 1 }, res);
			}

			var  amountBNSubFee;
			if(subtractFeeFromAmount){
				amountBNSubFee =  amountBN.minus(expectedFee);
			}else{
				amountBNSubFee = amountBN;
			}

			// Try to create transaction and get real fee
			transaction = transaction
				.from(suggest_utxos)          // Feed information about what unspent outputs one can use
				.to(toAddrString, amountBNSubFee.toNumber())  // Add an output with the given amount of satoshis
				.change(fromAddrString);      // Sets up a change address where the rest of the funds will go
			
			var realfee = transaction._getUnspentValue();
			if(subtractFeeFromAmount){
				amountBN =  amountBN.minus(realfee);
			}

			transaction.removeOutput(0);
			transaction.to(toAddrString, amountBN.toNumber());
			...
			var estimate_size = transaction._estimateSize();
			var expected_fee = estimate_size * _feePerKb / 1000;
			if((expected_fee > transaction._getUnspentValue()) && customFeeBN.isLessThanOrEqualTo(0)) {
				return self.common.handleErrors({ message: "Transaction Fee Error: Fee is too small", fee: transaction._getUnspentValue(), expected_fee: expected_fee, code: 1 }, res);
			} 

			var feeError = transaction._hasFeeError(serialize_opts, transaction._getUnspentValue());
			if (feeError) {
				return self.common.handleErrors({ message: feeError, fee: transaction._getUnspentValue(), code: 1 }, res);
			}
...
WalletController.prototype.createUnsigedRawTransactionMulti = function (req, res) {
	...
	this.node.estimateFee(nConfirmExpect, function (err, feeRateEstimate) {
		...
		var utxoCapBN = totalAmountBN.plus(expectedFee);
		...
		self.node.getAddressUnspentOutputs(fromAddrString, true, function (err, utxos) {
			...
			if(totalBN.isLessThan(totalAmountBN)){
				return self.common.handleErrors({ message: "Not enough money", code: 1 }, res);
			}
