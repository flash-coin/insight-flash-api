...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    reward: this.getBlockReward(info.height) / 1e10,
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.bitcoind.getBlockHashesByTimestamp(lte, gte, options, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getBlockReward = function(height) {
  ...
  // WL Fix for Flashcoin network; reward = 0;
  return 0
  if (halvings >= 64) {
    return 0;
  }
